    ...
    Store.findBy(req.params.name)
        .then(store =>{
            store.name === req.params.name
                ...
                : res.status(404).json({message: 'There is no store by this name available'});
...
router.post('/', async (req, res) =>{
    const store = {...req.body}
        try{
            ...
            res.status(201).json({message: 'Your store has been created.', storeData});
        } catch(error){
...
router.delete('/:id', (req, res) => {
        Store.remove(req.params.id)
            .then(removed => {
                removed
                    ? res.status(202).json({removed_shop_id: req.params.id, message: 'Store Has Been Deleted'})
                    :   res.send(404).json({message: `Could not find store with ID ${req.params.id}`}); 
