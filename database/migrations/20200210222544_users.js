...
exports.up = function (knex) {
    return knex.schema.createTable('users', tbl => {
        ...
        tbl.integer('role_id').notNullable().unsigned().unique().references('id').inTable('roles').onUpdate('CASCADE').onDelete('CASCADE');
        ...
        tbl.string('account_number', 255)
            .unique()
            .notNullable();

        tbl.string('routing_number', 255)
            .unique()
            .notNullable();

        tbl.string('card_number', 255)
            .unique()
            .notNullable();
